...
int main(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "o:b:")) != -1) {
        ...
        case 'o': // open and set outfile away from stdout
            ...
            if(outfile_fd < 0) {
                ERR_CLOSE("Could not open %s: %s", optarg);
            }
            outfile_name = optarg;
            break;
        case 'b': // parse for and set a size for our buffer
            ...
            if(buf_size <= 0) {
                ERR_CLOSE("Invalid buffer size provided: %s\n", optarg);
            }
            break;
        default:
            USAGE();
    ...
    do {
        ...
        while((bytes_read = read(infile_fd, io_buffer, buf_size)) > 0) {
            ...
            if(bytes_wrote != bytes_read) {
                ERR_CLOSE("Failed to write %s to %s: %s\n", argv[optind], 
                    outfile_name);
            }
        }
        
        if(infile_fd != STDIN_FILENO && close(infile_fd) < 0) {
            ERR_CLOSE("Failed to close %s: %s\n", argv[optind]);
        }
